...
const renameFiles = [
  ["src/library.ts", "src/ly-ts-axios.ts"],
  ["test/library.test.ts", "test/ly-ts-axios.test.ts"]
]
...
function modifyContents(libraryName: string, username: string, usermail: string) {
  ...
  try {
    const changes = replace.sync({
      files,
      from: [/ly-ts-axios/g, /--username--/g, /--usermail--/g],
...
function renameItems(libraryName: string) {
  ...
  renameFiles.forEach(function(files) {
    ...
    let newFilename = files[1].replace(/ly-ts-axios/g, libraryName)
    mv(
